...
func (r *loggingResource) process() {
	...
	if err := json.Unmarshal([]byte(configStr), &cfgs); err != nil {
		log.Warnf("logging_config_parse_failed pod=%s err=%s cfg=%s", r.parent, err, configStr)
		return
	}
...
func (r *loggingResource) shouldInject() (bool, string) {
	...
	if configStr := loggingMatchNamespaceOrLabelsForConfig(r.namespace, r.pod.Labels); configStr != "" {
		log.Debugf("logging_found_from_pod config=%s pod=%s", configStr, r.parent)
...
func (r *loggingResource) getVolumePairs(needVolumePaths []string) (volumeNames, volumePaths []string) {
	log.Debugf("logging_volume_paths=%s", needVolumePaths)
	...
	for idx := range needVolumePaths {
		...
		if exists {
			if volumeManager.IsEmptyDirVolume(name) {
				log.Infof("logging_reuse_volume name=%s path=%s pod=%s", name, needVolumePaths[idx], r.parent)
			} else {
				log.Warnf("logging_mount_conflict path=%s pod=%s not_emptydir=true", needVolumePaths[idx], r.parent)
				continue
			}
		} else {
