...
| --- | --- | --- |
| Node.js 18 to 25 | `{{.DDTraceNodeJSImage}}` | Default recommended version. It includes `dd-trace@5.102.0`, which requires `node >=18 <26` |
| Node.js 16 | `{{.DDTraceNodeJS16Image}}` | Use the `dd-trace` 4.x series. Do not use `5.102.0` for Node.js 16 |
...
| --- | --- | --- |
| Python 3.7 | `{{.DDTracePython37Image}}` | `ddtrace` 2.x supports Python 3.7. Python 3.7 is not supported by `ddtrace` 3.x/4.x |
| Python 3.8 | `{{.DDTracePython38Image}}` | `ddtrace` 3.x supports Python 3.8. `ddtrace` 4.x requires Python 3.9 or later |
| Python 3.9 to 3.14 | `{{.DDTracePythonImage}}` | Current `ddtrace` 4.x requires `python >=3.9,<3.15` |
