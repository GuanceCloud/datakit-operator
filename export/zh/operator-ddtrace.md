...
| --- | --- | --- |
| Node.js 18 到 25 | `{{.DDTraceNodeJSImage}}` | 默认推荐版本，内置 `dd-trace@5.102.0`，要求 `node >=18 <26` |
| Node.js 16 | `{{.DDTraceNodeJS16Image}}` | 使用 `dd-trace` 4.x 系列；不建议把 `5.102.0` 用于 Node.js 16 |
...
| --- | --- | --- |
| Python 3.7 | `{{.DDTracePython37Image}}` | `ddtrace` 2.x 支持 Python 3.7；Python 3.7 不支持 `ddtrace` 3.x/4.x |
| Python 3.8 | `{{.DDTracePython38Image}}` | `ddtrace` 3.x 支持 Python 3.8；`ddtrace` 4.x 要求 Python 3.9 及以上 |
| Python 3.9 到 3.14 | `{{.DDTracePythonImage}}` | `ddtrace` 4.x 当前要求 `python >=3.9,<3.15` |
