...
- `image_pull_policy` による注入イメージの pull ポリシー設定をサポート。未指定または不正な値はデフォルトの `Always` を使用（#110）

## 1.9.1(2026-09-09) {#cl-1.9.1}

- Kubernetes Lease に基づく DataKit の中央選挙をサポート。DataKit 2.12.0 以降との併用が必要（#109）
- Lease キャッシュの遅延により、新しい Leader のリース更新が誤って拒否される可能性がある問題を修正（#109）
- Pod キャッシュの同期をバックグラウンド処理に変更。準備が整うまで照会 API は 503 を返し、他の API の起動を妨げないように改善（#109）
- ブランチの Pipeline から、日付と commit のタグを付けた RC イメージを手動でリリースできるように対応（#108）

## 1.9.0(2026-08-26) {#cl-1.9.0}

- Java、Python、および Node.js アプリケーションへの OpenTelemetry 自動計装の注入をサポート（#98、#101、#102）
- 互換性のない設定がアプリケーション Pod に影響しないよう、計装注入の安全性と設定の耐障害性を改善（#87、#105）
- Admission Webhook の再呼び出し時に DDTrace Java が有効にならない場合がある問題を修正（#100）
